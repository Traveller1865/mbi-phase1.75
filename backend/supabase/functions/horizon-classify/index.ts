...
const METRIC_COLUMN: Record<string, keyof DailyInput> = {
  ...
  sleep_continuity_pct: "sleep_continuity_pct",
  steps:                "steps",
...
serve(async (req: Request) => {
  ...
  try {
    // ── 1. Fetch daily_inputs (most recent 97 days) ───────────────────────────
    // BUG FIX: ascending+limit fetches the OLDEST 97 rows when the user has
    // more than 97 days of history. We must fetch descending (most recent first)
    // then reverse to ascending order for the baseline/eval window slicing below.
    ...
    // Reverse to ascending date order so slice(-7) returns the most recent 7 days
    // and slice(0, -7) returns the baseline window (oldest to second-most-recent).
    const allDays = (inputs as DailyInput[]).reverse();
