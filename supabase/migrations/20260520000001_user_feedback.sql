...
create table if not exists public.user_feedback (
    ...
    date            text not null,
    score_accuracy  text check (score_accuracy  in ('accurate', 'off', 'inaccurate')),
    brief_quality   text check (brief_quality   in ('aligned', 'stale', 'unclear')),
    nudge_relevance text check (nudge_relevance in ('helpful', 'missed_the_mark', 'not_applicable')),
    note_text       text,
